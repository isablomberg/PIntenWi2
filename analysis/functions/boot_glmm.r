...
###########################################################################################################
###########################################################################################################
boot.lmer<-function(model.res, excl.warnings=F, nboots=1000, para=F, resol=1000, level=0.95, 
	...
	if(!is.list(use)){result=as.data.frame(result[[1]])}
	all.boots=matrix(unlist(all.res), nrow=nboots, byrow=T)
	colnames(all.boots)=colnames(all.res[[1]])
	ci.est=apply(all.boots, 2, quantile, prob=c((1-level)/2, 1-(1-level)/2), na.rm=T)
	if(length(fixef(model.res))>1){
	...
	return(list(ci.estimates=ci.est, ci.fitted=result, warnings=all.warns, ind.boots=all.boots))
}
